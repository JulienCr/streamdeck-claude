#!/usr/bin/env bash
# Creates a directory symlink in the Windows Stream Deck Plugins folder
# pointing at this plugin's `.sdPlugin/` directory inside WSL.
#
...
# `\\wsl.localhost\<distro>\...` target the SD app can read.
#
# Requires Windows Developer Mode enabled (or Administrator) so non-elevated
# `mklink /D` is allowed.
